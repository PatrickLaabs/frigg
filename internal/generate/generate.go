...
// ClusterctlConfig generates clusterctl config to frigg dir.
func ClusterctlConfig() {
	data := &clusterctlconfig{
...
// WorkloadManifest curls the template manifest for a capd workload cluster and stores it to the frigg working dir.
func WorkloadManifest() {
...
func KindConfigGen() {
	...
	newFilePath := filepath.Join(friggDir, vars.KindconfigName)
...
func MgmtArgoApps() {
	...
	url := "git@github.com:" + username + "/" + vars.FriggMgmtGitOpsName + ".git"
...
func MgmtArgoCD() {
	...
	if err != nil {
		println(color.RedString("error reading ssh private key: %v\n", err))
	}
...
func MgmtClusterApiOperator() {
	data := &clusterApiOperatorAutoGenerated{
		...
		Spec: struct {
			...
			Version:     consts.ClusterApiProviderHelmVersion,
			Options: struct {
				WaitForJobs bool   `yaml:"waitForJobs"`
				Wait        bool   `yaml:"wait"`
				Timeout     string `yaml:"timeout"`
